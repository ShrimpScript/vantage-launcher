...
/// The whole chain. Returns the profile and stashes the refresh token in the OS keychain.
pub async fn sign_in(http: &reqwest::Client, client_id: &str) -> Result<Account> {
    ...
    // 4. XSTS -> Minecraft. This is where an unapproved Azure app dies with a 403.
    let mc_res = http
        .post(MC_LOGIN)
        .json(&serde_json::json!({
